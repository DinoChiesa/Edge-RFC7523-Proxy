...
public class JwsTool extends JOSEToolBase {
  ...
  private static final String optString = "VGb:A:k:P:s:H:i:p:f:Dhv"; // getopt style
  ...
  public void run() throws Exception {
    ...
    if (toolAction == ToolAction.VERIFY) {
      ...
      if (contentFilename != null) {
        ...
        if (!m.find()) throw new IllegalStateException("Missing algorithm");
        String encodedPayload =
            Base64URL.encode(readAllBytes(Files.newInputStream(Paths.get(contentFilename))))
                .toString();
      ...
      if (wantDetach != null && wantDetach) {
        System.out.printf(
            "%s..%s\n",
            jwsObject.getHeader().toBase64URL().toString(), jwsObject.getSignature().toString());
      } else {
  ...
  public static void usage() {
    ...
    System.out.println(
        "     -f                   optional. file containing detached payload content.");
    System.out.println(
        "     -A <alg>             optional. Algorithm. Use {RS,HS,ES,PS}{256,384,512}. Default: RS256.");
    System.out.println(
        "     -k <publickeyfile>   optional. the PEM file containing the public key. For RS* or ES.\n");
    System.out.println(
    ...
    System.out.println(
        "     -i <keyid>           optional. The keyID (kid) to include in the JWS header.");
